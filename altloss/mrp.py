...
_state_map = {
    'RGT': {'RGD_VR2W', 'RGD-B?', 'RGUP', 'RG_UP', 'RG_DOWN', 'RADIAL GATES UPSTREAM', 'RADIAL GATES DOWNSTREAM'},
    'BAY': set(), # the forebay is not directly observable
    'IC':  {'IC1', 'IC1_FISHERMANS POINT', "IC1 FISHERMAN'S PT", 'IC2'},
    'LVR': {'IC3', 'IC3_VR2W', 'IC3 TRASH RACK', 'P1', 'P5', 'P4_A', 'P4_B', 'S1_UP', 'S1_DOWN', 'S2_UP_LEFT', 'S2_UP_RIGHT', 'S2_DOWN'},
    'HLD': {f'HTB{n}' for n in range(10)}, # there are multiple HTB hydrophones
    'FSB': {'FSB_A', 'FSB_B', 'FISH SCIENCE BUILDING'},
    'SVG': {'CLRS_A', 'CLRS_B', 'HBRS_A', 'HBRS_B', 'HORSESHOE BEND', 'CURTIS LANDING DOWN'},
...
_from_map = {
    'RGT': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'BAY': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'IC':  {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'LVR': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'HLD': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'FSB': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'SVG': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
    'DTH': {'RGT', 'BAY', 'IC', 'LVR', 'FSB', 'HLD'},
}
...
# columns in the data that hold timestamps and need to be
# converted to Pandas datetime type
_time_cols = ['DateTimeUTC-8']        

def assign_state(df, time_cols=_time_cols):
    ...
    df = df.rename(columns=dict((c, 'Time') for c in time_cols))
    df['Time'] = pd.to_datetime(df['Time'])
    df = df.sort_values(by=['Transmitter', 'Time'])
...
# for example, a fish could be observed at RGT and then
...
def add_bay_obs(df):
    df = add_intermediate_obs(df, 'RGT', 'IC', 'BAY')
    df = add_intermediate_obs(df, 'IC', 'RGT', 'BAY')
    return df

def add_death_obs(df):
    df.loc[df['NextState'].isna(), 'NextState'] = 'DTH'
    return df
...
def extract_one_obs(df, min_dt=pd.to_timedelta('10s')):
...
def load_stan_data(obs_file, tag_file, species, state_map=_stan_state_map):
    ...
    tags = tags.loc[tags['Species'].isin(species) & (tags['TagType'] == 'Acoustic')]
    tags = tags.reset_index(drop=True).reset_index()
    tags = tags.rename(columns={'index': 'Fish'})
    tags['Fish'] = tags['Fish'] + 1
    # add fish inices to observations
    obs = obs.merge(
        tags[['AcousticTag', 'Setup', 'Fish']], 
        left_on=['Setup', 'TransmitterName'],
        right_on=['Setup', 'AcousticTag'],
        how='inner'
    )
    ...
    data['N_transient'] = data['N_states'] - 2
    trans = [
        [
...
def draw_batch(data, fit, offset=0, size=1000):
